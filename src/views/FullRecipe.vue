...
<template>
    <div class="flex w-full flex-col py-8  px-10 min-h-screen bg-slate-100">
        <h2 class="text-slate-600 text-5xl font-bold uppercase py-8 text-center bg-slate-300 mb-7">{{ data.strMeal }}</h2>
        <div class=" flex items-start">
            <!-- <h4>{{ data }}</h4> -->
            <div class="flex justify-start items-start w-6/12 ">
                <!-- <div class="w-full"> -->
                <div class=" w-full flex justify-center flex-col ">
                    <div class="w-5/6 text-justify bg-slate-200">
                        <!-- <h4 class="hidden">{{ data.idMeal }}</h4> -->
                        <h4 class="text-2xl text-slate-600 font-bold pt-2">Recipe From: {{ data.strArea }}</h4>
                        <h4 class="text-2xl text-slate-600 font-bold pt-2">Category: {{ data.strCategory }}</h4>
                        <!-- <h4 class="hidden">{{ data.strMeal }}</h4> -->
                        <!-- <h4>{{ data.strInstructions }}</h4> -->
                        <!-- <h4 class="hidden">{{ data.strMealThumb }}</h4> -->
                        <h4 class="text-2xl text-slate-600 font-bold uppercase py-2">Tags: {{ data.strTags }}</h4>
                        <iframe :href="data.strYoutube" class="w-full h-24 bg-red-400 hidden"></iframe>
                    </div>
                    <div class="w-5/6 pt-7">
                        <img :src="data.strMealThumb" alt="img" class="w-fit rounded-md shadow-lg">
                    </div>
                    <div class="flex w-full py-12">
                        <table class="w-5/6 bg-slate-50 border shadow-lg uppercase">
                            <tr class="border-b border-slate-400 ">
                                <th
                                    class="text-lg border-b border-r  border-slate-400 font-medium text-slate-600 bg-slate-300 px-6 py-2 text-center">
                                    Ingredirnt</th>
                                <th class="text-lg font-medium text-slate-600 bg-slate-300 px-6 py-2 text-center">
                                    Measurement</th>
                            </tr>
                            <tr v-for="item in ingre" :key="item.key" class="border-b border-slate-400">
                                <td v-if="(item[0].length != 0)"
                                    class="text-base text-slate-600 bg-slate-100 border-r  border-slate-400 font-light px-6 py-2 whitespace-nowrap text-center">
                                    {{ item[0] }}</td>
                                <td v-if="(item[0].length != 0)"
                                    class="text-base text-slate-600 bg-slate-100  font-light px-6 py-2 whitespace-nowrap text-center">
                                    {{ item[1] }}</td>
                            </tr>
                        </table>
                    </div>
            ...
            <!--  -->
            <div class="flex flex-col  justify-start w-6/12 shadow-xl pb-8 px-12">
                <div class="w-full ">
                    <h2 class="text-4xl font-semibold  text-slate-600 bg-slate-200 py-4 text-center">How to prepare ?
                    </h2>
                    <p class="text-lg text-slate-700 py-12 text-justify">{{ data.strInstructions }}</p>
                </div>
                <a :href="data.strYoutube"
                    class="text-lg text-slate-50 font-semibold px-4 py-2 bg-red-600 rounded-lg shadow-xl text-center mb-12">YouTube</a>
            </div>
        </div>
    </div>
</template>
