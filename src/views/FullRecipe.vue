...
<template>
    <div v-if="loaded" class="flex w-full flex-col py-8  md:px-10 px-4 min-h-screen bg-slate-100">
        <h2 class="text-slate-600 md:text-5xl text-2xl font-bold uppercase py-8 text-center bg-slate-300 mb-7">{{
                data.strMeal
        }}</h2>
            ...
            <div class="flex justify-start items-start md:w-6/12 w-full ">
                ...
                <div class=" w-full flex justify-center flex-col ">
                    <div class="md:w-5/6 w-full text-justify bg-slate-200 px-4 text-ellipsis overflow-clip">
                        ...
                        <h4 class="md:text-2xl text-lg text-slate-600 font-bold pt-2">Recipe From: {{ data.strArea }}
                        </h4>
                        <h4 class="md:text-2xl text-lg text-slate-600 font-bold pt-2">Category: {{ data.strCategory }}
                        </h4>
                        <!-- <h4 class="hidden">{{ data.strMeal }}</h4> -->
                        <!-- <h4>{{ data.strInstructions }}</h4> -->
                        <!-- <h4 class="hidden">{{ data.strMealThumb }}</h4> -->
                        <h4 class="md:text-2xl text-lg text-slate-600 font-bold uppercase py-2 text-ellipsis">Tags: {{
                                data.strTags
                        }}</h4>
                        ...
                        <table class="md:w-5/6 w-full bg-slate-50 border shadow-lg uppercase ">
                            <tr class="border-b border-slate-400 ">
                                <th
                                    ...
                                    Ingredirnt</th>
                                <th
                                    class="text-lg font-medium text-slate-600 bg-slate-300 md:px-6 px-2 py-2 text-center">
                                    Measurement</th>
                            </tr>
